...
I chose to represent the tracker using a List<String>, where each string stores a record like "date | mileage mi | service". This makes the kernel methods easy to implement since I can add, remove, and access records using the list. The convention is that the list is not null and all records follow this format. The correspondence is that the list represents the sequence of maintenance records in order.
